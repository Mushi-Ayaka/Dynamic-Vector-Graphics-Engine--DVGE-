...
// Se ha eliminado la dependencia de 'remotion' (Easing) para abstraer @dvge/core
...
// Implementación interna de Bezier (reemplaza remotion Easing.bezier)
const cubicBezier = (p1x: number, p1y: number, p2x: number, p2y: number) => {
    return (t: number) => {
        // Implementación simplificada (suficiente para este contexto core). 
        // Para precisión perfecta se requiere resolver el polinomio de Newton-Raphson.
        const cx = 3.0 * p1x;
...
export const dvUtils = {
    bezier: (curveParams: string | number[], t: number): number => {
        try {
            ...
            return t; // Fallback a linear easing
        }
    },
